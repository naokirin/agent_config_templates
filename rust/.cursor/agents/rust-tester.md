...
5. Keep changes consistent with project rules (rust-style, rust-idioms). Do not change behaviour beyond what is needed to fix the failure. In tests, `.unwrap()` or `.expect()` is acceptable for clearer failure location.
